...
function AdminProfile() {
  ...
  const getAdminById = async () => {
    ...
    const response = await axios.get(`${process.env.REACT_APP_URL}:3001/profile/admin/${adminUserId}`);
  ...
  const updateAdmin = async (e) => {
    ...
    //alert(userId);
    try {
      await axios.patch(`${process.env.REACT_APP_URL}:3001/profile/admin/${userId}`, {
        firstName,
        lastName,
        username,
        email,
